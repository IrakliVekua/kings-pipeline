...
import { calculateWeightedPipeline } from './pipeline.js';
...
// --- Defaults (used if Supabase not configured) ---
...
const SAMPLE_CARDS = [
  {
    id: uuid(),
    country: 'Moldova',
    org: 'Local Olympiad Assoc.',
    owner: 'Roma',
    nextAction: 'Kickoff call notes → NDA',
    priority: 'Med',
    value: 12000,
    flags: { nda: false, tech: false, jaa: false },
  },
  {
    id: uuid(),
    country: 'Egypt',
    org: 'STEM Competitions Egypt',
    owner: 'Irakli',
    nextAction: 'Share JAA draft',
    priority: 'High',
    value: 30000,
    flags: { nda: true, tech: true, jaa: false },
  },
  {
    id: uuid(),
    country: 'Cyprus',
    owner: 'Ana',
    nextAction: 'Post Upwork job for scout',
    priority: 'Low',
    value: 8000,
    flags: { nda: false, tech: false, jaa: false },
  },
...
export default function KingsPipelineKanban() {
      ...
      if (data.demo) {
        ...
        if (saved) {
          try {
            ...
            setStages(parsed.stages || DEFAULT_STAGES);
  ...
  const onDragStart = (e, fromStageId, cardId) => {
    e.dataTransfer.setData(
      'text/plain',
      JSON.stringify({ t: 'card', from: fromStageId, id: cardId })
    );
  ...
  const moveStage = (index, dir) => {
    ...
    s[j] = tmp;
    setStages(s);
    saveStageOrder(BOARD_ID, s);
  };

  const pipeline = React.useMemo(
    () => calculateWeightedPipeline(stages, columns, probabilityMode),
    [stages, columns, probabilityMode]
  );
  ...
  const [pendingNew, setPendingNew] = React.useState({
      ...
      <div className="mx-auto max-w-[1500px] px-4 py-6">
        ...
        <div className="sticky top-0 z-10 -mx-4 mb-6 bg-white/70 dark:bg-zinc-900/70 backdrop-blur supports-[backdrop-filter]:bg-white/50 dark:supports-[backdrop-filter]:bg-zinc-900/50 border-b px-4 py-3">
          <div className="flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-semibold tracking-tight">
                  Kings Expansion Pipeline
                </h1>
                <div className="text-[12px] text-zinc-500">
                  Weighted: <b>${pipeline.weighted.toLocaleString()}</b> • Face
            ...
            <div className="flex items-center gap-2">
              <button
                className="rounded-xl px-3 py-2 text-sm border hover:bg-zinc-50 dark:hover:bg-zinc-800"
              ...
              >
                <option value="absolute">
                  Win% = chance to reach First Event
                </option>
                <option value="transition">
                  Win% = chance to move to next stage
                </option>
              </select>
              <input
                className="w-56 rounded-xl border border-zinc-300 dark:border-zinc-700 bg-white dark:bg-zinc-900 px-3 py-2 text-sm"
                placeholder="Search…"
                value={filter}
              ...
              <details className="relative">
                <summary className="list-none">
                  <span className="rounded-xl px-3 py-2 text-sm border inline-block cursor-pointer hover:bg-zinc-50 dark:hover:bg-zinc-800">
                    Import
                  </span>
                </summary>
              ...
              <button
                ...
                className="rounded-xl px-3 py-2 text-sm bg-indigo-600 text-white hover:bg-indigo-700 shadow-sm"
              >
                New country ⌘N
              </button>
            </div>
        ...
        {demo && (
          <div className="mb-4 text-yellow-700">
            Supabase not configured; using local demo board.
          </div>
        )}
        ...
        <div className="flex gap-2 overflow-x-auto mb-4">
          {pipeline.perStage.map((s) => (
            <span
              key={s.id}
              className="inline-flex items-center gap-2 rounded-full border px-3 py-1 text-[12px] bg-white/70 dark:bg-zinc-900/70"
            >
            ...
            <div key={stage.id} className="min-w-[340px] max-w-[360px]">
              <div className="flex items-center justify-between mb-2">
                ...
                <div className="flex items-center gap-1">
                  <button
                    className="px-2 py-1 rounded-lg border text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800"
              ...
              <div
                className="rounded-2xl border bg-white/90 dark:bg-zinc-900/80 backdrop-blur p-3 min-h-[160px] shadow-sm"
                onDragOver={onDragOverCol}
                onDrop={(e) => onDropToCol(e, stage.id)}
              >
                {(filteredColumns[stage.id] || []).length === 0 && (
                  <div className="text-xs text-zinc-500 py-6 text-center border border-dashed rounded-xl">
                    Drop country here
                  </div>
                )}
                    ...
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          ...
                          <div className="flex items-center gap-2 text-xs text-zinc-500">
                            {c.owner && (
                              <span className="inline-flex items-center gap-1">
                                <span className="h-5 w-5 rounded-full bg-indigo-100 text-indigo-700 grid place-items-center text-[10px] font-semibold">
                                  {initials(c.owner)}
                                </span>{' '}
                                {c.owner}
                              </span>
                          ...
                          {c.nextAction && (
                            <p className="text-sm mt-2">→ {c.nextAction}</p>
                          )}
                        ...
                        <div className="flex items-center gap-2">
                          <button
                            className="px-2 py-1 rounded-lg border text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800"
                            onClick={() => setEditingCard({ stageId: stage.id, card: c })}
                          >
                            Edit
                          </button>
                          <button
                            className="px-2 py-1 rounded-lg border text-xs hover:bg-zinc-50 dark:hover:bg-zinc-800"
                            onClick={() => removeCard(stage.id, c.id)}
                          >
                            Delete
                          </button>
                        </div>
        ...
        {showStageEditor && (
          <StageEditor
            ...
            onSave={(rows) => {
              setColumns((prev) => {
                const next = {};
                for (const s of rows) next[s.id] = prev[s.id] || [];
                return next;
              });
              setStages(rows);
              setShowStageEditor(false);
              saveStageOrder(BOARD_ID, rows);
            }}
          />
        )}
        ...
        {editingCard && (
          <EditCardModal
            ...
            onSave={(c) => {
              updateCard(editingCard.stageId, c);
              setEditingCard(null);
            }}
          />
        )}
...
function StageEditor({ stages, onSave, onClose }) {
  ...
  const add = () =>
    setRows((r) => [...r, { id: uuid(), name: 'New stage', prob: 0 }]);
      ...
      <div className="w-full max-w-2xl rounded-2xl border bg-white dark:bg-zinc-900">
        <div className="p-4 border-b">
          ...
          <p className="text-sm text-zinc-500 mt-1">
            Rename, set Win%, WIP and reorder. Save to apply.
          </p>
        </div>
        <div className="p-4 space-y-2 max-h-[60vh] overflow-auto">
          ...
          <div className="flex justify-between mt-3">
            ...
            <div className="flex gap-2">
              <button className="px-3 py-2 rounded-xl border text-sm" onClick={onClose}>
                Close
              </button>
              <button className="px-3 py-2 rounded-xl border text-sm" onClick={() => onSave(rows)}>
                Save
              </button>
            </div>
          </div>
...
function EditCardModal({ card, onSave, onClose }) {
  ...
  const toggleFlag = (k) => (value) =>
    setForm({ ...form, flags: { ...(form.flags || {}), [k]: value } });
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-2xl border bg-white dark:bg-zinc-900">
        <div className="p-4 border-b">
          <h3 className="text-lg font-semibold">Edit Country</h3>
        </div>
        <div className="p-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-3">
            ...
            <div>
              ...
              <select
                ...
                value={form.priority || ''}
                onChange={(e) =>
                  setForm({ ...form, priority: e.target.value || undefined })
                }
              >
                <option value="">—</option>
                <option>High</option>
            ...
            <div className="md:col-span-2">
              ...
              <textarea
                ...
                rows={4}
                {...bind('notes')}
                placeholder="Context, risks, decision log…"
              />
            </div>
            <div className="flex items-center gap-4 md:col-span-2 text-sm">
              <label className="inline-flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!(form.flags && form.flags.nda)}
                  onChange={(e) => toggleFlag('nda')(e.target.checked)}
                />{' '}
                NDA signed
              </label>
              <label className="inline-flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!(form.flags && form.flags.tech)}
                  onChange={(e) => toggleFlag('tech')(e.target.checked)}
                />{' '}
                Tech meeting done
              </label>
              <label className="inline-flex items-center gap-2">
                <input
                  type="checkbox"
                  checked={!!(form.flags && form.flags.jaa)}
                  onChange={(e) => toggleFlag('jaa')(e.target.checked)}
                />{' '}
                JAA signed
              </label>
            </div>
    ...
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4">
      <div className="w-full max-w-xl rounded-2xl border bg-white dark:bg-zinc-900 shadow">
        <div className="p-4 border-b flex items-center justify-between">
          ...
          <button className="px-2 py-1 rounded-lg border text-xs" onClick={onClose}>
            Close
          </button>
        </div>
...
(function runTests() {
  try {
    ...
    console.assert(
      Math.abs(rA.weighted - (100 * 0.5 + 200 * 1)) < 1e-6,
      'Test 1 failed: absolute mode basic'
    );
    ...
    console.assert(
      Math.abs(rB.weighted - (100 * 0.5 + 200 * 1)) < 1e-6,
      'Test 2 failed: transition mode basic'
    );
    ...
    console.assert(
      Math.abs(rC.weighted - 90) < 1e-6,
      'Test 3 failed: transition chain'
    );
    ...
    console.assert(
      Math.abs(rD.weighted - 110) < 1e-6,
      'Test 4 failed: absolute chain'
    );
    ...
    console.assert(
      !Number.isNaN(rE.total) && rE.total === 5,
      'Test 5 failed: handle missing/zero values'
    );

    // Test 6: zero probabilities
    const stagesC = [
      { id: 's1', name: 's1', prob: 0 },
      { id: 's2', name: 's2', prob: 0 },
      { id: 's3', name: 's3', prob: 100 },
    ];
    const columnsD = {
      s1: [{ id: '1', value: 50 }],
